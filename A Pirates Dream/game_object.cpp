...
GameObject::GameObject(const glm::vec3 &position, Geometry *geom, Shader *shader, GLuint texture, int type) 
{
...
{ 
    delete explosion_timer_; 
}

int GameObject::GetTimer(void) const
{
    //return false;
    return explosion_timer_->Finished(); 
}

void GameObject::SetTimer(float end_time)
{
    explosion_timer_->Start(end_time);
}
